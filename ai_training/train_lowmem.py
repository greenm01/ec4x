#!/usr/bin/env python3
"""
Memory-optimized training for 16GB VRAM.
Uses gradient checkpointing and smaller batches to fit in limited VRAM.
"""
...
class TrainingConfig:
    ...
    output_dir: str = "models/ec4x-mistral-7b-lowmem"

    # Training hyperparameters (optimized for 16GB VRAM)
    num_train_epochs: int = 3
    per_device_train_batch_size: int = 1  # Minimum
    gradient_accumulation_steps: int = 32  # Effective batch size = 1 * 32 = 32
    ...
    max_seq_length: int = 384  # Shorter to save memory
    ...
    # Checkpointing
    save_steps: int = 200
    save_total_limit: int = 2  # Keep fewer checkpoints to save disk space
    ...
    num_trainable_layers: int = 2  # Only train last 2 layers to reduce memory
...
def train(config: TrainingConfig):
    """Main training loop."""
    print("=" * 70)
    print("EC4X Model Training (Memory Optimized)")
    print("=" * 70)
    print()
    ...
    training_args = TrainingArguments(
        ...
        dataloader_pin_memory=False,  # Disable pin_memory for ROCm
    )
