...
* Toggle views: `L`
* Sorting: `S` (cycles columns, press twice to reverse direction)
* Filters: `1-4` (All, Idle, In Transit, Needs Attention)
* Search: `/` (matches fleet ID or sector coordinates)
* Batch: `X` to select fleets, `C` for batch command, `R` for batch ROE

Columns (120 cols target):
* Flt (Fleet ID with prefix glyphs)
* Location
* Sect (sector coordinates)
* Ships
* AS
* DS
* CMD
