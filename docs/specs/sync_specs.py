...
def generate_spacelift_table(
    facilities_config: Dict[str, Any], ships_config: Dict[str, Any]
) -> str:
...
def generate_prestige_table(config: Dict[str, Any]) -> str:
    ...
    prestige_mapping = [
        ...
        (
            "espionage",
            "assassination",
            "Assassination Detected (penalty)",
            "AssassinationDetected",
        ),
        (
            "espionage",
            "low_impact_sabotage",
            "Espionage Attempt Failed (penalty)",
            "EspionageFailure",
        ),
        (
            "military",
            "destroy_squadron",
            "Major Ship Destroyed (per ship)",
            "ShipDestroyed",
        ),
...
def generate_espionage_actions_table(config: Dict[str, Any]) -> str:
    ...
    for toml_key, enum_name, effect in espionage_mapping:
        ...
        lines.append(
            f"| {toml_key.replace('_', ' ').title()} | `{enum_name}` | {ebp_cost} | {detection_base}% | {effect} |"
        )

    return "\n".join(lines)


def generate_espionage_prestige_table(
    prestige_config: Dict[str, Any], espionage_config: Dict[str, Any]
) -> str:
    ...
    actions = [
        (
            ...
            "Tech Theft",
            "tech_theft_ebp",
            "Attempt to steal critical R&D tech.",
        ),
        (
            "low_impact_sabotage",
            "low_impact_sabotage_victim",
            "Sabotage (Low Impact)",
            "sabotage_low_ebp",
            "Small-scale sabotage to a colony's industry.",
        ),
        (
            "high_impact_sabotage",
            "high_impact_sabotage_victim",
            "Sabotage (High Impact)",
            "sabotage_high_ebp",
            "Major sabotage to a colony's industry.",
        ),
        (
            "assassination",
            "assassination_victim",
            "Assassination",
            "assassination_ebp",
            "Attempt to eliminate a key figures within the target House.",
        ),
        (
            "cyber_attack",
            "cyber_attack_victim",
            "Cyber Attack",
            "cyber_attack_ebp",
            "Attempt to hack into a Starbase's systems to cause damage and chaos.",
        ),
        (
            "economic_manipulation",
            "economic_manipulation_victim",
            "Economic Manipulation",
            "economic_manipulation_ebp",
            "Influence markets to harm the target's economy",
        ),
        (
            "psyops_campaign",
            "psyops_campaign_victim",
            "Psyops Campaign",
            "psyops_campaign_ebp",
            "Launch a misinformation campaign or demoralization effort.",
        ),
        (
            "counter_intel_sweep",
            "counter_intel_sweep_victim",
            "Counter-Intelligence Sweep",
            "counter_intel_sweep_ebp",
            "Defensive operation to block enemy intelligence gathering.",
        ),
        (
            "intelligence_theft",
            "intelligence_theft_victim",
            "Intelligence Theft",
            "intelligence_theft_ebp",
            "Steal target's entire intelligence database.",
        ),
        (
            "plant_disinformation",
            "plant_disinformation_victim",
            "Plant Disinformation",
            "plant_disinformation_ebp",
...
def generate_tax_incentive_table(
    prestige_config: Dict[str, Any], economy_config: Dict[str, Any]
) -> str:
...
def generate_iu_investment_table(economy_config: Dict[str, Any]) -> str:
    ...
    lines.append(
        f"| Up to 50%               | {tier_1_mult}             | {tier_1_pp}  |"
    )
    ...
    lines.append(
        f"| 151% and above          | {tier_5_mult}             | {tier_5_pp}  |"
    )
...
def generate_population_transfer_cost_table(population_config: Dict[str, Any]) -> str:
    ...
    for display_name, key in planets:
        ...
        lines.append(
            f"| {display_name:<12} | {base_cost:<18} | Base × (1 + {distance_modifier} × jumps) |"
        )

    lines.append("")
    lines.append(
        f"*Distance modifier: +{int(distance_modifier * 100)}% per jump beyond first"
    )
    lines.append("")
    lines.append(
...
def generate_population_transfer_mechanics_table(
    ...
    lines.append(
        f"| **Transit Time** | {turns_per_jump} turn/jump | Minimum {min_turns} turn even within same system |"
    )
    ...
    lines.append(
        f"| **Source Reserve** | {min_source} PU | Source colony must retain at least this |"
    )
    lines.append(
        f"| **Concurrent Limit** | {max_concurrent} transfers | Maximum active transfers per house |"
    )
    ...
    lines.append(
        f"| **Source Conquered** | {source_behavior.title()} | Transfer proceeds (Guild is neutral) |"
    )
    lines.append(
        f"| **Dest Conquered** | {dest_behavior.title()} | PTUs lost (refugees disperse) |"
    )
    lines.append(
        f"| **Dest Blockaded** | {blockade_behavior.title()} | PTUs return to source |"
    )
...
def generate_maintenance_shortfall_table(prestige_config: Dict[str, Any]) -> str:
    ...
    for turn in range(1, 5):
        ...
        if turn == 4:
            ...
            lines.append(
                f"| {turn_label:<39} | {penalty:>23} per turn | –{abs(cumulative)}, –{abs(next_cumulative)}, etc. {' ':<6}|"
            )
        else:
...
def generate_tech_level_table(
    tech_config: Dict[str, Any], tech_name: str, max_levels: int = 5
) -> str:
    ...
    for level in range(1, max_levels + 1):
        ...
        trp_str = (
            f"\\*{trp}"
            if level == max_levels
            and tech_name in ["construction_tech", "weapons_tech"]
            else str(trp)
        )
...
def generate_terraforming_upgrade_cost_table(tech_config: Dict[str, Any]) -> str:
    ...
    for display_name, key in planets:
        ...
        lines.append(
            f"| {display_name:<12} | {ter_str:<12} | {pu_str:<9} | {pp_str:<4} |"
        )
...
def generate_fighter_doctrine_table(tech_config: Dict[str, Any]) -> str:
    ...
    for level in range(1, 4):
        ...
        lines.append(
            f"| {level_str:<10} | {prereq:<12} | {trp_str:<8} | {sl_req:<11} | {capacity_str:<19} |"
        )
...
def generate_aco_table(tech_config: Dict[str, Any]) -> str:
    ...
    for level in range(1, 4):
        ...
        lines.append(
            f"| {level_str:<10} | {prereq:<12} | {trp_str:<8} | {sl_req:<11} | {cv_cap} FS        | {cx_cap} FS        |"
        )
...
def generate_starting_tech_table(tech_config: Dict[str, Any]) -> str:
    ...
    fd_capacity = tech_config.get("fighter_doctrine", {}).get(
        "level_1_capacity_multiplier", 1.0
    )
    aco_cv = tech_config.get("advanced_carrier_operations", {}).get(
        "level_1_cv_capacity", 3
    )
    aco_cx = tech_config.get("advanced_carrier_operations", {}).get(
        "level_1_cx_capacity", 5
    )
    ...
    tech_entries = [
        (
            "**EL**",
            starting_tech.get("economic_level", 1),
            "Economic Level - 5% production bonus (GCO modifier)",
        ),
        (
            "**SL**",
            starting_tech.get("science_level", 1),
            "Science Level - Base research capability",
        ),
        (
            "**CST**",
            starting_tech.get("construction_tech", 1),
            "Construction Tech - Base shipyard capacity",
        ),
        (
            "**WEP**",
            starting_tech.get("weapons_tech", 1),
            "Weapons Tech - Base combat strength (AS/DS)",
        ),
        (
            "**TER**",
            starting_tech.get("terraforming_tech", 1),
            "Terraforming Tech - Base terraforming capability",
        ),
        (
            "**ELI**",
            starting_tech.get("electronic_intelligence", 1),
            "Electronic Intelligence - Base spy scout capability",
        ),
        (
            "**CLK**",
            starting_tech.get("cloaking_tech", 1),
            "Cloaking Tech - Stealth and detection avoidance",
        ),
        (
            "**SLD**",
            starting_tech.get("shield_tech", 1),
            "Shield Tech - Planetary shield technology (bombardment defense)",
        ),
        (
            "**CIC**",
            starting_tech.get("counter_intelligence", 1),
            "Counter-Intelligence - Base espionage detection",
        ),
        (
            "**FD**",
            starting_tech.get("fighter_doctrine", 1),
            f"Fighter Doctrine I - {fd_capacity}x fighter capacity multiplier",
        ),
        (
            "**ACO**",
            starting_tech.get("advanced_carrier_ops", 1),
            f"Advanced Carrier Operations I - CV={aco_cv}FS, CX={aco_cx}FS",
        ),
    ]
...
def replace_inline_values_diplomacy(
    content: str, diplomacy_config: Dict[str, Any]
) -> str:
    ...
    replacements = {
        ...
        "LOW_SAB_DICE": lambda: diplomacy_config["espionage_effects"][
            "low_sabotage_dice"
        ],
        "HIGH_SAB_DICE": lambda: diplomacy_config["espionage_effects"][
            "high_sabotage_dice"
        ],
        "ASSASSIN_REDUCTION": lambda: str(
            int(
                diplomacy_config["espionage_effects"]["assassination_srp_reduction"]
                * 100
            )
        ),
        "ASSASSIN_TURNS": lambda: "one"
        if diplomacy_config["espionage_effects"]["assassination_duration_turns"] == 1
        else str(diplomacy_config["espionage_effects"]["assassination_duration_turns"]),
        "ECON_DISRUPT_TURNS": lambda: "one"
        if diplomacy_config["espionage_effects"]["economic_disruption_duration_turns"]
        == 1
        else str(
            diplomacy_config["espionage_effects"]["economic_disruption_duration_turns"]
        ),
        "PROPAGANDA_REDUCTION": lambda: str(
            int(diplomacy_config["espionage_effects"]["propaganda_tax_reduction"] * 100)
        ),
        "PROPAGANDA_TURNS": lambda: "one"
        if diplomacy_config["espionage_effects"]["propaganda_duration_turns"] == 1
        else str(diplomacy_config["espionage_effects"]["propaganda_duration_turns"]),
        "FAILED_ESPIONAGE_PRESTIGE": lambda: str(
            abs(diplomacy_config["detection"]["failed_espionage_prestige_loss"])
        ),
    }
...
def update_diplomacy_spec(
    espionage_prestige_table: str,
    cic_modifier_table: str,
    cic_threshold_table: str,
    diplomacy_config: Dict[str, Any],
):
...
def update_economy_spec(
    raw_table: str,
    tax_penalty_table: str,
    tax_incentive_table: str,
    iu_table: str,
    colonization_table: str,
    maintenance_shortfall_table: str,
    pop_transfer_cost_table: str,
    pop_transfer_mechanics_table: str,
    el_table: str,
    sl_table: str,
    cst_table: str,
    wep_table: str,
    ter_table: str,
    ter_upgrade_table: str,
    eli_table: str,
    clk_table: str,
    sld_table: str,
    cic_table: str,
    ...
    # Replace inline values first
    content = replace_inline_values(
        content, economy_config, construction_config, military_config, tech_config
    )
    ...
    markers = {
        ...
        "MAINTENANCE_SHORTFALL_TABLE": (
            maintenance_shortfall_table,
            "maintenance shortfall penalty table",
        ),
        "POPULATION_TRANSFER_COST_TABLE": (
            pop_transfer_cost_table,
            "population transfer cost table",
        ),
        "POPULATION_TRANSFER_MECHANICS_TABLE": (
            pop_transfer_mechanics_table,
            "population transfer mechanics table",
        ),
        ...
        "TER_UPGRADE_COST_TABLE": (
            ter_upgrade_table,
            "Terraforming upgrade cost table",
        ),
...
def replace_inline_values(
    ...
    replacements = {
        ...
        "PU_TO_PTU_CONVERSION": lambda: str(
            economy_config["population"]["pu_to_ptu_conversion"]
        ),
        "TAX_WINDOW": lambda: str(
            economy_config["tax_mechanics"]["tax_averaging_window_turns"]
        ),
        ...
        "FIGHTER_IU_DIVISOR": lambda: str(
            military_config["fighter_mechanics"]["fighter_capacity_iu_divisor"]
        ),
        "STARBASE_PER_FS": lambda: str(
            military_config["fighter_mechanics"]["starbase_per_fighter_squadrons"]
        ),
        "CAPACITY_GRACE_PERIOD": lambda: str(
            military_config["fighter_mechanics"]["capacity_violation_grace_period"]
        ),
        ...
        "SQUADRON_PU_DIVISOR": lambda: str(
            military_config["squadron_limits"]["squadron_limit_pu_divisor"]
        ),
        "SQUADRON_MINIMUM": lambda: str(
            military_config["squadron_limits"]["squadron_limit_minimum"]
        ),
        "BREAKTHROUGH_BASE": lambda: f"{int(economy_config['research']['research_breakthrough_base_chance'] * 100)}%",
        "BREAKTHROUGH_RP_PER_PERCENT": lambda: str(
            economy_config["research"]["research_breakthrough_rp_per_percent"]
        ),
        "MINOR_BREAKTHROUGH_BONUS": lambda: str(
            economy_config["research"]["minor_breakthrough_bonus"]
        ),
        ...
        "REV_STEALTH_BONUS": lambda: str(
            economy_config["research"]["revolutionary_stealth_detection_bonus"]
        ),
        ...
        "EL_EARLY_INCREMENT": lambda: str(
            economy_config["research"]["el_early_increment"]
        ),
        "EL_LATE_INCREMENT": lambda: str(
            economy_config["research"]["el_late_increment"]
        ),
        "SRP_BASE_COST": lambda: str(economy_config["research"]["srp_base_cost"]),
        "SRP_SL_MULTIPLIER": lambda: str(
            economy_config["research"]["srp_sl_multiplier"]
        ),
        "SL_EARLY_BASE": lambda: str(economy_config["research"]["sl_early_base"]),
        "SL_EARLY_INCREMENT": lambda: str(
            economy_config["research"]["sl_early_increment"]
        ),
        "SL_LATE_INCREMENT": lambda: str(
            economy_config["research"]["sl_late_increment"]
        ),
        "TRP_FIRST_LEVEL_COST": lambda: str(
            economy_config["research"]["trp_first_level_cost"]
        ),
        "TRP_LEVEL_INCREMENT": lambda: str(
            economy_config["research"]["trp_level_increment"]
        ),
        ...
        "MILITARY_SHIP_TURNS": lambda: "two"
        if construction_config["construction"]["shipyard_turns"] == 2
        else str(construction_config["construction"]["shipyard_turns"]),
        "SPACELIFT_SHIP_TURNS": lambda: "one"
        if construction_config["construction"]["spaceport_turns"] == 1
        else str(construction_config["construction"]["spaceport_turns"]),
        "SHIP_REPAIR_TURNS": lambda: "one"
        if construction_config["repair"]["ship_repair_turns"] == 1
        else str(construction_config["repair"]["ship_repair_turns"]),
        ...
        "SHIP_REPAIR_COST_DECIMAL": lambda: str(
            construction_config["repair"]["ship_repair_cost_multiplier"]
        ),
    }
...
def replace_inline_values_operations(
    content: str,
    ...
    replacements = {
        ...
        "BLOCKADE_PRESTIGE": lambda: str(
            combat_config["blockade"]["blockade_prestige_penalty"]
        ),
...
def replace_inline_values_assets(
    content: str,
    espionage_config: Dict[str, Any],
    construction_config: Dict[str, Any],
    military_config: Dict[str, Any],
    starmap_config: Dict[str, Any],
) -> str:
    ...
    replacements = {
        ...
        "MESH_6_PLUS": lambda: str(
            espionage_config["scout_detection"]["mesh_6_plus_scouts"]
        ),
        "STARBASE_ELI_BONUS": lambda: str(
            espionage_config["scout_detection"]["starbase_eli_bonus"]
        ),
        "DOMINANT_TECH_THRESHOLD": lambda: str(
            int(espionage_config["scout_detection"]["dominant_tech_threshold"] * 100)
        ),
        "MAX_ELI": lambda: str(espionage_config["scout_detection"]["max_eli_level"]),
        "THRESHOLD_DICE": lambda: str(
            espionage_config["raider_detection"]["threshold_variance_dice"]
        ),
        "ELI_ADVANTAGE_MAJOR": lambda: str(
            espionage_config["raider_detection"]["eli_advantage_major"]
        ),
        "ELI_ADVANTAGE_MINOR": lambda: str(
            espionage_config["raider_detection"]["eli_advantage_minor"]
        ),
        "FIGHTER_SQUADRON_COST": lambda: str(
            construction_config["costs"]["fighter_squadron_cost"]
        ),
        "CAPACITY_GRACE_PERIOD": lambda: str(
            military_config["fighter_mechanics"]["capacity_violation_grace_period"]
        ),
        # Starmap configuration values
        "MAJOR_LANE_PERCENT": lambda: str(
            int(starmap_config["lane_weights"]["major_weight"] * 100)
        ),
        "MINOR_LANE_PERCENT": lambda: str(
            int(starmap_config["lane_weights"]["minor_weight"] * 100)
        ),
        "RESTRICTED_LANE_PERCENT": lambda: str(
            int(starmap_config["lane_weights"]["restricted_weight"] * 100)
        ),
        "HOMEWORLD_LANE_COUNT": lambda: str(
            starmap_config["homeworld_placement"]["homeworld_lane_count"]
        ),
    }
...
def update_operations_spec(
    ...
    # Replace inline values first
    content = replace_inline_values_operations(
        content, combat_config, construction_config, military_config
    )
...
def update_assets_spec(
    spy_detection_table: str,
    raider_detection_table: str,
    espionage_config: Dict[str, Any],
    construction_config: Dict[str, Any],
    military_config: Dict[str, Any],
    starmap_config: Dict[str, Any],
    ...
    # Replace inline values first
    content = replace_inline_values_assets(
        content, espionage_config, construction_config, military_config, starmap_config
    )
...
def replace_inline_values_gameplay(
    content: str, gameplay_config: Dict[str, Any], game_setup_config: Dict[str, Any]
) -> str:
    ...
    replacements = {
        "STARTING_PRESTIGE": lambda: str(
            game_setup_config["starting_resources"]["starting_prestige"]
        ),
        "VICTORY_PRESTIGE": lambda: str(
            game_setup_config["victory_conditions"]["prestige_threshold"]
        ),
        "DEFENSIVE_COLLAPSE_TURNS": lambda: num_to_word(
            gameplay_config["elimination"]["defensive_collapse_turns"]
        ),
        "MIA_TURNS": lambda: num_to_word(
            gameplay_config["autopilot"]["mia_turns_threshold"]
        ),
        ...
        "HOMEWORLD_COLONY_LEVEL": lambda: "V"
        if game_setup_config["homeworld"]["colony_level"] == 5
        else str(game_setup_config["homeworld"]["colony_level"]),
        "HOMEWORLD_PU": lambda: str(game_setup_config["homeworld"]["population_units"]),
        "STARTING_TREASURY": lambda: str(
            game_setup_config["starting_resources"]["treasury"]
        ),
        "DEFAULT_TAX_RATE": lambda: str(
            int(game_setup_config["starting_resources"]["default_tax_rate"] * 100)
        ),
        "STARTING_SPACEPORTS": lambda: num_to_word(
            game_setup_config["starting_facilities"]["spaceports"]
        ),
        "STARTING_SHIPYARDS": lambda: num_to_word(
            game_setup_config["starting_facilities"]["shipyards"]
        ),
        "STARTING_ETAC": lambda: num_to_word(
            game_setup_config["starting_fleet"]["etac"]
        ),
        "STARTING_LIGHT_CRUISER": lambda: "a"
        if game_setup_config["starting_fleet"]["light_cruiser"] == 1
        else str(game_setup_config["starting_fleet"]["light_cruiser"]),
        "STARTING_DESTROYERS": lambda: num_to_word(
            game_setup_config["starting_fleet"]["destroyer"]
        ),
        "STARTING_SCOUTS": lambda: num_to_word(
            game_setup_config["starting_fleet"]["scout"]
        ),
        "STARTING_EL": lambda: str(
            game_setup_config["starting_tech"]["economic_level"]
        ),
        "STARTING_SL": lambda: str(game_setup_config["starting_tech"]["science_level"]),
        "STARTING_CST": lambda: str(
            game_setup_config["starting_tech"]["construction_tech"]
        ),
        "STARTING_WEP": lambda: str(game_setup_config["starting_tech"]["weapons_tech"]),
        "STARTING_TER": lambda: str(
            game_setup_config["starting_tech"]["terraforming_tech"]
        ),
        "STARTING_ELI": lambda: str(
            game_setup_config["starting_tech"]["electronic_intelligence"]
        ),
        "STARTING_CIC": lambda: str(
            game_setup_config["starting_tech"]["counter_intelligence"]
        ),
    }
...
def update_gameplay_spec(
    gameplay_config: Dict[str, Any], game_setup_config: Dict[str, Any]
):
    ...
    content = replace_inline_values_gameplay(
        content, gameplay_config, game_setup_config
    )
...
def main():
    ...
    espionage_prestige_table = generate_espionage_prestige_table(
        prestige_config, espionage_config
    )
    ...
    pop_transfer_mechanics_table = generate_population_transfer_mechanics_table(
        population_config
    )
    ...
    update_operations_spec(
        shield_table, combat_config, construction_config, military_config
    )
    update_assets_spec(
        spy_detection_table,
        raider_detection_table,
