...
import ../../engine/systems/capacity/carrier_hangar
...
proc totalDepositPP(deposits: ResearchDeposits): int =
  int(deposits.erp) + int(deposits.srp) + int(deposits.mrp)
...
proc poolDeposit(deposits: ResearchDeposits, pool: ResearchPoolIdx): int32 =
  ...
  of ResearchPoolIdx.PoolMRP: deposits.mrp
...
proc setPoolDeposit(deposits: var ResearchDeposits, pool: ResearchPoolIdx, value: int32) =
  ...
  of ResearchPoolIdx.PoolMRP: deposits.mrp = value
...
proc setAllocForItem(alloc: var ResearchAllocation, item: ResearchItem, val: int32) =
  ...
  of ResearchItemKind.MilitaryLevel: alloc.military = val
...
proc nextResearchPoolIdx(pool: ResearchPoolIdx): ResearchPoolIdx =
  case pool
  of ResearchPoolIdx.PoolERP: ResearchPoolIdx.PoolMRP
  of ResearchPoolIdx.PoolMRP: ResearchPoolIdx.PoolSRP
  of ResearchPoolIdx.PoolSRP: ResearchPoolIdx.PoolERP

proc prevResearchPoolIdx(pool: ResearchPoolIdx): ResearchPoolIdx =
  case pool
  of ResearchPoolIdx.PoolERP: ResearchPoolIdx.PoolSRP
  of ResearchPoolIdx.PoolMRP: ResearchPoolIdx.PoolERP
  of ResearchPoolIdx.PoolSRP: ResearchPoolIdx.PoolMRP
...
proc clampAllAllocations(model: var TuiModel) =
  ...
  for item in researchItems():
    ...
    if curLvl >= progressionMaxLevel(item):
      ...
      continue
    if research_projection.isBlockedProjected(
        levels,
        points,
        model.ui.researchDeposits,
        model.ui.researchPurchases,
        item,
        gho):
      setAllocForItem(model.ui.researchAllocations, item, 0)
      continue
  # Second pass: clamp per-pool totals to available pool RP
  for poolIdx in [ResearchPoolIdx.PoolERP, ResearchPoolIdx.PoolSRP, ResearchPoolIdx.PoolMRP]:
...
proc updateResearchPurchases(model: var TuiModel) =
  ...
  for item in researchItems():
    ...
    of ResearchItemKind.MilitaryLevel: model.ui.researchPurchases.military = true
...
proc adjustTechAllocation(model: var TuiModel, delta: int) =
  ...
  let poolRP      = research_projection.projectedPoolRP(
                      points, model.ui.researchDeposits, pool, gho, levels)
...
proc resetFleetDetailSubModal(model: var TuiModel) =
  ...
  model.ui.fleetDetailModal.selectedCarrierShipId = none(ShipId)
...
proc colonyCarrierLoadPool(model: TuiModel, sourceFleet: Fleet): int =
  if int(sourceFleet.location) notin model.view.ownColoniesBySystem:
    return 0
  model.view.ownColoniesBySystem[int(sourceFleet.location)].fighterIds.len

proc carrierLoadCapacity(model: TuiModel, ship: Ship): int =
  let acoLevel = if model.view.techLevels.isSome:
      max(1'i32, model.view.techLevels.get().aco)
    else:
      1'i32
  let maxCapacity = carrierMaxCapacity(ship.shipClass, acoLevel)
  max(0, int(maxCapacity) - ship.embarkedFighters.len)
...
proc fighterQtyMax(model: TuiModel): int =
  ...
  of ZeroTurnCommandType.UnloadFighters:
    if model.ui.fleetDetailModal.selectedCarrierShipId.isNone:
      return 0
    let carrierId = int(model.ui.fleetDetailModal.selectedCarrierShipId.get())
    if carrierId notin model.view.ownShipsById:
      return 0
    model.view.ownShipsById[carrierId].embarkedFighters.len
  of ZeroTurnCommandType.TransferFighters:
    if model.ui.fleetDetailModal.selectedCarrierShipId.isNone:
      return 0
    let carrierId = int(model.ui.fleetDetailModal.selectedCarrierShipId.get())
...
proc setFighterQuantityInput(model: var TuiModel, qty: int) =
  model.ui.fleetDetailModal.fighterQuantityInput.clear()
  let clampedQty = max(0, min(qty, fighterQtyMax(model)))
  if clampedQty == 0:
    return
  for ch in $clampedQty:
...
proc buildCarrierPickerCandidatesForZtc(
  ...
  model.ui.fleetDetailModal.carrierPickerCandidates = @[]
  model.ui.fleetDetailModal.carrierPickerIdx = 0
  let ztcType = model.ui.fleetDetailModal.ztcType
  let colonyLoadPool = model.colonyCarrierLoadPool(sourceFleet)
  model.ui.fleetDetailModal.carrierPickerPoolCount = colonyLoadPool
  for shipId in sourceFleet.ships:
    ...
    if ship.shipClass notin {ShipClass.Carrier, ShipClass.SuperCarrier}:
      continue
    if ztcType.isSome and ztcType.get() == ZeroTurnCommandType.LoadFighters:
      let availableSpace = min(
        model.carrierLoadCapacity(ship),
        colonyLoadPool
      )
      if availableSpace <= 0:
        continue
      ...
      model.ui.fleetDetailModal.carrierPickerCandidates.add(CarrierPickerRow(
        shipId: ship.id,
        classLabel: $ship.shipClass,
        maxCount: ship.embarkedFighters.len,
        stagedCount: 0,
        fighterIds: ship.embarkedFighters
      ))
  if model.ui.fleetDetailModal.carrierPickerCandidates.len == 0:
    if showEmptyStatus:
      if ztcType.isSome and ztcType.get() == ZeroTurnCommandType.LoadFighters:
        model.ui.statusMessage = "No carrier space available"
      else:
...
proc refreshCarrierPickerCandidates(model: var TuiModel) =
  ...
  var sourceFleet = Fleet()
  if sourceFleetId in model.view.ownFleetsById:
    sourceFleet = model.view.ownFleetsById[sourceFleetId]
  let colonyLoadPool = model.colonyCarrierLoadPool(sourceFleet)
  for row in model.ui.fleetDetailModal.carrierPickerCandidates:
    ...
    if isLoad:
      ...
      refreshed.add(CarrierPickerRow(
        ...
        classLabel: $ship.shipClass,
        maxCount: ship.embarkedFighters.len,
        stagedCount: clamp(row.stagedCount, 0, ship.embarkedFighters.len),
        fighterIds: ship.embarkedFighters
      ))
  ...
  model.ui.fleetDetailModal.carrierPickerPoolCount =
    max(0, colonyLoadPool - totalStaged)
  if model.ui.fleetDetailModal.carrierPickerCandidates.len == 0:
    model.ui.fleetDetailModal.carrierPickerIdx = 0
  else:
...
proc setCarrierPickerQty(
  ...
  var maxQty = row.maxCount
  if isLoad:
    var usedElsewhere = 0
    for idx, existing in model.ui.fleetDetailModal.carrierPickerCandidates:
      if idx != rowIdx:
        usedElsewhere += existing.stagedCount
    let colonyPool = model.colonyCarrierLoadPool(
      model.view.ownFleetsById.getOrDefault(model.ui.fleetDetailModal.fleetId)
    )
...
proc selectionAcceptor*(model: var TuiModel, proposal: Proposal) =
  ...
  of ActionKind.listUp:
    ...
    if model.ui.mode == ViewMode.Fleets and
      ...
      if model.ui.researchFocus == ResearchFocus.Pools:
        model.ui.researchPoolIdx =
          prevResearchPoolIdx(model.ui.researchPoolIdx)
  ...
  of ActionKind.listDown:
    ...
    if model.ui.mode == ViewMode.Fleets and
      ...
      if model.ui.researchFocus == ResearchFocus.Pools:
        model.ui.researchPoolIdx =
          nextResearchPoolIdx(model.ui.researchPoolIdx)
      elif model.ui.researchFocus == ResearchFocus.Detail:
        discard
      else:
        model.ui.selectedIdx = model.nextSelectableResearchIdx(
  ...
  of ProposalKind.pkGameAction:
    ...
    of ActionKind.researchLiquidateConfirm:
      if model.ui.liquidationConfirmActive and model.ui.liquidationAmount > 0:
        ...
        of ResearchPoolIdx.PoolMRP:
          model.ui.stagedLiquidation.mrp += model.ui.liquidationAmount
      ...
      if model.ui.liquidationConfirmActive:
        if model.view.researchPoints.isSome:
          ...
          let poolRP = case model.ui.liquidationPool
            ...
            of ResearchPoolIdx.PoolMRP: pts.mrp
    ...
    of ActionKind.entryCreatePasswordAppend:
      if proposal.gameActionData.len > 0:
        if model.ui.entryModal.createPasswordFocus == 0:
          ...
          discard model.ui.entryModal.confirmPasswordInput.appendChar(
            proposal.gameActionData[0])
    of ActionKind.entryCreatePasswordBackspace:
      if model.ui.entryModal.createPasswordFocus == 0:
        model.ui.entryModal.createPasswordInput.backspace()
      else:
        model.ui.entryModal.confirmPasswordInput.backspace()
    ...
    of ActionKind.entryCreatePasswordTab:
      model.ui.entryModal.createPasswordFocus =
        1 - model.ui.entryModal.createPasswordFocus
    of ActionKind.entryDismissWarning:
      model.ui.entryModal.dismissSecurityWarning()
    of ActionKind.entryKeyDetail:
      model.ui.entryModal.showKeyDetail = not model.ui.entryModal.showKeyDetail
      if model.ui.entryModal.showKeyDetail:
        model.ui.entryModal.keyDetailMasked = true
    of ActionKind.entryKeyDetailMask:
      model.ui.entryModal.keyDetailMasked =
        not model.ui.entryModal.keyDetailMasked
    ...
    of ActionKind.entryDelete:
      if model.ui.entryModal.mode == EntryModalMode.PasswordPrompt:
        ...
        if model.ui.entryModal.createPasswordFocus == 0:
          ...
          model.ui.entryModal.confirmPasswordInput.delete()
      elif model.ui.entryModal.mode == EntryModalMode.ChangePasswordPrompt:
    ...
    of ActionKind.entryCursorLeft:
      if model.ui.entryModal.mode == EntryModalMode.PasswordPrompt:
        ...
        if model.ui.entryModal.createPasswordFocus == 0:
          ...
          model.ui.entryModal.confirmPasswordInput.moveCursorLeft()
      elif model.ui.entryModal.mode == EntryModalMode.ChangePasswordPrompt:
    ...
    of ActionKind.entryCursorRight:
      if model.ui.entryModal.mode == EntryModalMode.PasswordPrompt:
        ...
        if model.ui.entryModal.createPasswordFocus == 0:
          ...
          model.ui.entryModal.confirmPasswordInput.moveCursorRight()
      elif model.ui.entryModal.mode == EntryModalMode.ChangePasswordPrompt:
...
proc fleetDetailModalAcceptor*(model: var TuiModel, proposal: Proposal) =
  ...
  of ActionKind.fleetDetailListUp:
    if model.ui.fleetDetailModal.subModal == FleetSubModal.CommandPicker:
      ...
      let maxIdx = max(0,
        model.ui.fleetDetailModal.carrierPickerCandidates.len - 1)
      if model.ui.fleetDetailModal.carrierPickerIdx > 0:
        model.ui.fleetDetailModal.carrierPickerIdx -= 1
      else:
        model.ui.fleetDetailModal.carrierPickerIdx = maxIdx
      ...
      if current > 0:
        model.setFighterQuantityInput(current - 1)
  ...
  of ActionKind.fleetDetailListDown:
    if model.ui.fleetDetailModal.subModal == FleetSubModal.CommandPicker:
      ...
      let maxIdx = model.ui.fleetDetailModal.carrierPickerCandidates.len - 1
      if model.ui.fleetDetailModal.carrierPickerIdx < maxIdx:
        model.ui.fleetDetailModal.carrierPickerIdx += 1
      elif maxIdx >= 0:
        model.ui.fleetDetailModal.carrierPickerIdx = 0
    elif model.ui.fleetDetailModal.subModal == FleetSubModal.CargoParams:
  ...
  of ActionKind.fleetDetailQtyInc:
    if model.ui.fleetDetailModal.subModal == FleetSubModal.CarrierPicker:
      ...
      if idx >= 0 and idx < model.ui.fleetDetailModal.carrierPickerCandidates.len:
        ...
        model.setCarrierPickerQty(idx, row.stagedCount + 1)
  ...
  of ActionKind.fleetDetailSelectCommand:
    if model.ui.fleetDetailModal.subModal == FleetSubModal.CommandPicker:
      ...
      let candidates = model.ui.fleetDetailModal.carrierPickerCandidates
      let idx = model.ui.fleetDetailModal.carrierPickerIdx
      if model.ui.fleetDetailModal.ztcType.isSome and
          model.ui.fleetDetailModal.ztcType.get() in {
            ZeroTurnCommandType.LoadFighters,
        ...
        let ztcType = model.ui.fleetDetailModal.ztcType.get()
        var stagedAny = false
        var colonyFighterIds =
          model.view.ownColoniesBySystem[int(sourceFleet.location)].fighterIds
        for carrier in candidates:
          ...
          if carrier.stagedCount <= 0:
            model.replaceFighterLogisticsCommand(scope, none(ZeroTurnCommand))
            continue
          var fighterIds: seq[ShipId] = @[]
          if ztcType == ZeroTurnCommandType.LoadFighters:
            let loadCount = min(carrier.stagedCount, colonyFighterIds.len)
            if loadCount > 0:
              fighterIds = colonyFighterIds[0 ..< loadCount]
              if loadCount >= colonyFighterIds.len:
                colonyFighterIds = @[]
              else:
                colonyFighterIds =
                  colonyFighterIds[loadCount .. colonyFighterIds.high]
          else:
            fighterIds = carrier.fighterIds
            if carrier.stagedCount < fighterIds.len:
              fighterIds = fighterIds[0 ..< carrier.stagedCount]
          if fighterIds.len == 0:
            model.replaceFighterLogisticsCommand(scope, none(ZeroTurnCommand))
            continue
          let cmd = ZeroTurnCommand(
          ...
          stagedAny = true
        if not stagedAny:
          let actionName = if ztcType == ZeroTurnCommandType.LoadFighters:
              "load"
            else:
              "unload"
          model.ui.statusMessage = "Set a " & actionName & " quantity first"
          return
        model.ui.statusMessage =
          if ztcType == ZeroTurnCommandType.LoadFighters:
            "Staged Load Fighters"
          else:
            "Staged Unload Fighters"
        model.ztcCloseToFleets()
      else:
        if idx < 0 or idx >= candidates.len:
          model.ui.statusMessage = "No carrier selected"
          return
        model.ui.fleetDetailModal.selectedCarrierShipId =
          some(candidates[idx].shipId)
        model.ui.fleetDetailModal.fighterQuantityInput.clear()
      ...
      let ztcType = model.ui.fleetDetailModal.ztcType.get()
      let houseId = HouseId(model.view.viewingHouse.uint32)
      let maxQty = model.fighterQtyMax()
      let rawQty = parseInputQuantity(
        model.ui.fleetDetailModal.fighterQuantityInput
      )
      let qty = if rawQty == 0: maxQty else: min(rawQty, maxQty)
      var sourceCarrier = model.ui.fleetDetailModal.selectedCarrierShipId
      if ztcType == ZeroTurnCommandType.LoadFighters and sourceCarrier.isNone:
        for shipId in sourceFleet.ships:
          if int(shipId) notin model.view.ownShipsById:
            continue
          let ship = model.view.ownShipsById[int(shipId)]
      ...
      if proposal.kind == ProposalKind.pkGameAction:
        ...
        if ch >= '0' and ch <= '9':
          ...
          if idx >= 0 and
            ...
            let row = model.ui.fleetDetailModal.carrierPickerCandidates[idx]
            var candidate = $row.stagedCount
            if row.stagedCount == 0:
              candidate = ""
            candidate.add(ch)
            let qty = if candidate.len == 0: 0 else: parseInt(candidate)
            model.setCarrierPickerQty(idx, qty)
    elif model.ui.fleetDetailModal.subModal == FleetSubModal.FighterParams:
      if proposal.kind == ProposalKind.pkGameAction:
        let ch = if proposal.gameActionData.len > 0:
          proposal.gameActionData[0] else: '\0'
        if ch >= '0' and ch <= '9':
          var candidate = model.ui.fleetDetailModal.fighterQuantityInput.value()
          if candidate == "0":
            candidate = ""
          candidate.add(ch)
          var tempInput = initTextInputState()
          tempInput.setText(candidate)
          model.setFighterQuantityInput(parseInputQuantity(tempInput))
  else:
    discard
