...
proc keyToDirection(key: Key): Option[HexDirection] =
  ...
  case key
  of Key.Right: some(HexDirection.East)
  of Key.Left: some(HexDirection.West)
  of Key.Up:
  ...
  else: none(HexDirection)

# -----------------------------------------------------------------------------
# Navigation handlers
# -----------------------------------------------------------------------------

proc handleKey*(state: var HexMapState, event: KeyEvent,
                mapData: MapData): NavResult =
  ...
  if key == Key.Home or 
     (key == Key.Rune and event.rune == Rune('h')):
...
proc cycleToNextColony*(state: var HexMapState, colonies: seq[HexCoord]) =
  ## Move cursor to next owned colony in list
  if colonies.len == 0:
    return
  ...
  let nextIdx = if currentIdx < 0: 0
                else: (currentIdx + 1) mod colonies.len
  
  state.cursor = colonies[nextIdx]

proc cycleToPrevColony*(state: var HexMapState, colonies: seq[HexCoord]) =
  ## Move cursor to previous owned colony in list
  if colonies.len == 0:
    return
  ...
  let prevIdx = if currentIdx <= 0: colonies.len - 1
                else: currentIdx - 1
  
  state.cursor = colonies[prevIdx]

proc findHomeworld*(mapData: MapData): Option[HexCoord] =
  ## Find the homeworld for the viewing player
  for coord, sys in mapData.systems.pairs:
    if sys.isHomeworld and sys.owner.isSome and 
       sys.owner.get() == mapData.viewingHouse:
      return some(coord)
  none(HexCoord)
...
proc processNavigation*(state: var HexMapState, event: KeyEvent,
                        mapData: MapData): NavResult =
