...
proc crippleShip*(sq: var Squadron, index: int, ships: var Ships): bool =
  ...
  if index == -1:
    var flagship = ships.entities.entity(sq.flagshipId).get
  ...
  let shipId = sq.ships[index]
  var ship = ships.entities.entity(shipId).get
  if ship.isCrippled:
    return false
