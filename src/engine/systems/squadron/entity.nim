...
proc crippleShip*(sq: var Squadron, index: int, ships: var Ships): bool =
  ...
  if index == -1:
    var flagship = ships.entity(sq.flagshipId).get
  ...
  let shipId = sq.ships[index]
  var ship = ships.entity(shipId).get
  if ship.isCrippled:
    return false
