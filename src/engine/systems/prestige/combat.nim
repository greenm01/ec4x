...
proc awardCombatPrestige*(
  ...
  let victoryPrestige = applyMultiplier(
  ...
  if taskForceDestroyed:
    let tfPrestige = applyMultiplier(
  ...
  if squadronsDestroyed > 0:
    let squadronPrestige = applyMultiplier(
      getPrestigeValue(PrestigeSource.SquadronDestroyed)
    ) * squadronsDestroyed
    result.victorEvents.add(createPrestigeEvent(
  ...
  if forcedRetreat:
    let retreatPrestige = applyMultiplier(
