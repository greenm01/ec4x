...
import ../../types/[
  capacity, core, game_state, squadron, ship, production, event, colony,
  house, facilities
]
...
proc analyzeCapacity*(state: GameState, houseId: HouseId): capacity.CapacityViolation =
  ...
  result = capacity.CapacityViolation(
    ...
    entity: capacity.EntityIdUnion(kind: capacity.CapacityType.CapitalSquadron,
houseId: houseId),
...
proc planEnforcement*(state: GameState, violation: capacity.CapacityViolation): capacity.EnforcementAction =
  ...
  result.description = $toScrapCount & " capital squadron(s) auto-scrapped for " &
                      $violation.entity.houseId & " (IU loss, 50% salvage paid)"
...
proc applyEnforcement*(state: var GameState, action: capacity.EnforcementAction,
  ...
  if totalSalvage > 0:
    ...
    if houseOpt.isSome:
      ...
      logger.logDebug("Military", "Salvage credited to house treasury",
            " house=", $houseId,
            " amount=", $totalSalvage)

  logger.logDebug("Military", "Capital squadron capacity enforcement complete",
        " house=", $houseId,
...
proc processCapacityEnforcement*(state: var GameState,
  ...
  logger.logDebug("Military", "Capital squadron violations found, count=", $violations.len)
  ...
  if enforcementActions.len > 0:
    logger.logDebug("Military", "Enforcing capital squadron capacity violations, count=", $enforcementActions.len)
