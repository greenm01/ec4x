...
proc generateStarbaseSurveillance*(
  ...
  if not hasOperationalStarbase:
    return
      none(intel_types.StarbaseSurveillanceReport) # Crippled starbases can't surveil

  # Get surveillance sector (own system ONLY)
  var surveillanceSector: seq[SystemId] = @[starbaseSystemId]

  var detectedFleets:
    seq[tuple[fleetId: FleetId, location: SystemId, owner: HouseId, shipCount: int]] =
    @[]
  ...
  for fleetId, fleet in state.fleets:
    if fleet.location in surveillanceSector and fleet.owner != starbaseOwner:
      ...
      for squadron in fleet.squadrons:
        ...
        if squadron.flagship.stats.specialCapability.startsWith("ELI") and
            not squadron.flagship.isCrippled:
          hasScouts = true

        # Check for raiders (CLK capability)
        if squadron.flagship.stats.specialCapability.startsWith("CLK") and
            not squadron.flagship.isCrippled:
  ...
  events.add(
    event_factory.starbaseSurveillanceDetection(
      starbaseId = starbaseId,
      owner = starbaseOwner,
      systemId = starbaseSystemId,
...
proc processAllStarbaseSurveillance*(
    state: var GameState, turn: int, rng: var Rand, events: var seq[res_types.GameEvent]
) =
  ...
  for systemId, colony in state.colonies:
    if colony.starbases.len > 0:
      var surveillance =
        generateStarbaseSurveillance(state, systemId, colony.owner, turn, rng, events)
      ...
      if surveillance.isSome:
        ...
        if corruptionEffect.isSome:
          ...
          var corruptedDetected: seq[
            tuple[fleetId: FleetId, location: SystemId, owner: HouseId, shipCount: int]
          ] = @[]
