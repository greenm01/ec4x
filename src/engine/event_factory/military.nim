...
proc bombardment*(
  ...
  event.GameEvent(
    ...
    description:
      ...
      &"{infrastructureDamage} IU damaged, {populationKilled} PU " & &"killed",
    systemId: some(systemId),
    message:
      &"Infrastructure: {infrastructureDamage} IU, Population: " &
  ...
  event.GameEvent(
    ...
    houseId:
      if attackers.len > 0:
        ...
        none(HouseId),
    description:
      &"Battle at system {systemId} between " &
      &"{attackers.len} attacker(s) and {defenders.len} " & &"defender(s)",
    systemId: some(systemId),
...
proc combatPhaseCompleted*(
  ...
  event.GameEvent(
    ...
    description:
      ...
      &"({phaseRounds} rounds, {casualties.len} houses with " & &"casualties)",
    systemId: some(systemId),
    phase: some(phase),
    roundNumberPhase: some(roundNumber),
...
proc raiderDetected*(
  ...
  event.GameEvent(
    ...
    description:
      ...
      &"{detectorType} at system {systemId} (ELI {eliRoll} vs CLK " & &"{clkRoll})",
    systemId: some(systemId),
...
proc fighterEngagement*(
  ...
  event.GameEvent(
    ...
    description:
      ...
      &"{targetSquadron} at system {systemId} ({damage} damage, no " & &"CER roll)",
    systemId: some(systemId),
...
proc weaponFired*(
  ...
  event.GameEvent(
    ...
    description:
      ...
      &"squadron {targetSquadron} (CER {cerRoll}+{cerModifier}, " & &"{damage} damage)",
    systemId: some(systemId),
    sourceHouseId: some(attackerHouse),
    targetHouseId: some(targetHouse),
...
proc fleetRetreat*(
  ...
  event.GameEvent(
    ...
    description:
      ...
      &"(reason: {reason}, threshold: {threshold}, casualties: " & &"{casualties})",
    systemId: some(systemId),
    fleetId: some(fleetId),
...
proc shieldActivated*(
  ...
  event.GameEvent(
    ...
    description:
      ...
      &"{systemId} (roll {roll} vs {threshold}, blocked " & &"{percentBlocked}% of hits)",
    systemId: some(systemId),
  ...
  event.GameEvent(
    ...
    houseId:
      if attackers.len > 0:
        ...
        none(HouseId),
    description:
      &"Ground combat at system {systemId}: attackers rolled " &
...
proc starbaseCombat*(
  ...
  event.GameEvent(
    ...
    description:
      ...
      &"{targetSquadron} (AS {starbaseAs}, DS {starbaseDs}, " & &"+{eliBonus} ELI)",
    systemId: some(systemId),
