...
proc createGroundUnit*(state: var GameState, owner: HouseId, colonyId: ColonyId, unitType: GroundUnitType): GroundUnit =
  ...
  let newUnit = GroundUnit(id: unitId, unitType: unitType, owner: owner, attackStrength: 5, defenseStrength: 5, state: CombatState.Undamaged)
...
proc destroyGroundUnit*(state: var GameState, unitId: GroundUnitId) =
  ...
  for col in state.allColonies():
    case unit.unitType:
      ...
      of GroundUnitType.Marine:
        if unitId in col.marineIds: ownerColonyId = col.id
      of GroundUnitType.GroundBattery:
        if unitId in col.groundBatteryIds: ownerColonyId = col.id
      else: discard
    if ownerColonyId != 0'u32: break

  if ownerColonyId != 0'u32:
    var colony = gs_helpers.getColony(state, ownerColonyId).get()
    case unit.unitType:
      of GroundUnitType.Army:
        colony.armyIds.keepIf(proc(id: GroundUnitId): bool = id != unitId)
      of GroundUnitType.Marine:
        colony.marineIds.keepIf(proc(id: GroundUnitId): bool = id != unitId)
      of GroundUnitType.GroundBattery:
        colony.groundBatteryIds.keepIf(proc(id: GroundUnitId): bool = id != unitId)
      else: discard
