...
proc queueConstructionProject*(state: var GameState, colonyId: ColonyId, project: var ConstructionProject): ConstructionProject =
  ...
  if project.facilityId.isSome and project.facilityType.isSome:
    ...
    state.constructionProjects.byFacility.mgetOrPut((facilityType, facilityId), @[]).add(projectId)
    case facilityType:
...
proc completeConstructionProject*(state: var GameState, projectId: ConstructionProjectId) =
  ...
  if projectOpt.isNone: return
  let project = projectOpt.get()

  if state.constructionProjects.byColony.contains(project.colonyId):
    state.constructionProjects.byColony[project.colonyId].keepIf(proc(id: ConstructionProjectId): bool = id != projectId)
  ...
  if project.facilityId.isSome and project.facilityType.isSome:
    ...
    if state.constructionProjects.byFacility.contains(key):
      state.constructionProjects.byFacility[key].keepIf(proc(id: ConstructionProjectId): bool = id != projectId)
    
    case facilityType:
      of FacilityType.Spaceport:
        var spaceport = gs_helpers.getSpaceport(state, SpaceportId(facilityId)).get()
        spaceport.activeConstructions.keepIf(proc(id: ConstructionProjectId): bool = id != projectId)
        state.spaceports.entities.updateEntity(SpaceportId(facilityId), spaceport)
      of FacilityType.Shipyard:
        var shipyard = gs_helpers.getShipyard(state, ShipyardId(facilityId)).get()
        shipyard.activeConstructions.keepIf(proc(id: ConstructionProjectId): bool = id != projectId)
        state.shipyards.entities.updateEntity(ShipyardId(facilityId), shipyard)
      else: discard
  else:
...
proc queueRepairProject*(state: var GameState, colonyId: ColonyId, project: var RepairProject): RepairProject =
  ...
  if project.facilityId.isSome:
    ...
    state.repairProjects.byFacility.mgetOrPut((facilityType, facilityId), @[]).add(projectId)
...
proc completeRepairProject*(state: var GameState, projectId: RepairProjectId) =
  ...
  if state.repairProjects.byColony.contains(project.colonyId):
    state.repairProjects.byColony[project.colonyId].keepIf(proc(id: RepairProjectId): bool = id != projectId)
  ...
  if project.facilityId.isSome:
    ...
    if state.repairProjects.byFacility.contains(key):
      state.repairProjects.byFacility[key].keepIf(proc(id: RepairProjectId): bool = id != projectId)

    case facilityType:
      of FacilityType.Drydock:
        var drydock = gs_helpers.getDrydock(state, DrydockId(facilityId)).get()
        drydock.activeRepairs.keepIf(proc(id: RepairProjectId): bool = id != projectId)
        state.drydocks.entities.updateEntity(DrydockId(facilityId), drydock)
      else: discard
