...
proc resolveMaintenancePhase*(state: var GameState,
  ...
  logInfo(LogCategory.lcOrders, &"  Standing commands: {standingOrdersGenerated} orders generated")

  logInfo(LogCategory.lcOrders, &"[MAINTENANCE STEP 1a] Order activation complete ({state.fleetCommands.len} total orders)")
  ...
  logInfo(LogCategory.lcOrders, &"[MAINTENANCE STEP 1c] Moving fleets toward order targets... ({state.fleetCommands.len} persistent orders)")
