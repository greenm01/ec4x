...
type
  DiagnosticMetrics* = object ## Metrics collected per house, per turn
    ...
    houseId*: HouseId
    totalSystemsOnMap*: int32
      # Total systems on the starmap (constant, same for all houses)
    ...
    infrastructureDamageTotal*: int32
      # Total IU lost to bombardment/sabotage (cumulative)
    ...
    fighterCapacityMax*: int32
      # Max FS allowed (sum per colony: floor(Colony_IU/fighter_capacity_iu_divisor) × FD multiplier)
    fighterCapacityUsed*: int32 # Actual FS count (current)
    fighterCapacityViolation*: bool # Over capacity?
    squadronLimitMax*: int32
      # Max capital squadrons allowed (floor(Total_IU/squadron_limit_iu_divisor) × 2)
    ...
    campaigns_abandoned_captured*: int32
      # Campaigns abandoned - target taken by other (cumulative)
    ...
    domestikosRequirementsFulfilled*: int32
      # Requirements fulfilled by Treasurer mediation
  ...
  DiagnosticSession* = object ## Collection of all diagnostics for a game session
    gameId*: string
    seed*: int64
    numHouses*: int32
    mapSize*: int32
