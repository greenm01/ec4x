...
# =============================================================================
# Scout Network Events
# =============================================================================

proc scoutMeshNetworkFormed*(
  houseId: HouseId,
  fleetIds: seq[FleetId],
  totalScouts: int,
  eliBonus: int,
  systemId: SystemId
): event_types.GameEvent =
  ## Create event for scout mesh network formation
  let fleetList = fleetIds.mapIt($it).join(", ")
  event_types.GameEvent(
    eventType: event_types.GameEventType.ScoutMeshNetworkFormed,
    turn: 0,
    houseId: some(houseId),
    description: &"Scout mesh network formed at {systemId}: " &
                 &"{totalScouts} scouts (+{eliBonus} ELI bonus) " &
                 &"from fleets: {fleetList}",
    systemId: some(systemId),
    meshFleetIds: some(fleetIds),
    totalScouts: some(totalScouts),
    meshEliBonus: some(eliBonus),
    meshLocation: some(systemId)
  )

proc spyScoutDeployed*(
  spyScoutId: string,
  houseId: HouseId,
  missionType: string,
  ...
  ## Create event for spy scout deployment
  event_types.GameEvent(
    eventType: event_types.GameEventType.SpyScoutDeployed,
    turn: 0,
    houseId: some(houseId),
    description: &"Spy scout {spyScoutId} deployed for {missionType} mission: " &
                 &"target {targetHouse} at {targetSystem}",
    systemId: some(originSystem),
    spyScoutId: some(spyScoutId),
    deployingHouseId: some(houseId),
    spyMissionType: some(missionType),
    spyTargetSystem: some(targetSystem),
    spyTargetHouse: some(targetHouse)
  )
...
proc spyScoutTravel*(
  ...
  ## Create event for spy scout travel
  event_types.GameEvent(
    eventType: event_types.GameEventType.SpyScoutTravel,
    turn: 0,
    houseId: some(houseId),
    description: &"Spy scout {spyScoutId} traveling: {origin} -> {destination} " &
                 &"({progress}/{totalJumps} jumps)" &
                 (if detectionRisk: " [HIGH DETECTION RISK]" else: ""),
    systemId: some(origin),
    travelingSpyScoutId: some(spyScoutId),
    travelOrigin: some(origin),
    travelDestination: some(destination),
