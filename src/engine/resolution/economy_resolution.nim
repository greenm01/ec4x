...
proc resolveMaintenancePhase*(state: var GameState, events: var seq[GameEvent], orders: Table[HouseId, OrderPacket]): seq[econ_types.CompletedProject] =
  ...
  logInfo(LogCategory.lcEconomy, &"Maintenance phase complete: {result.len} projects ready for commissioning")
