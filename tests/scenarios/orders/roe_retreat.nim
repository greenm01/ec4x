...
when isMainModule:
  ...
  echo "✅ 3/4 tests passing"
  echo "❌ 1/4 test failing (Conservative ROE scenario)"
  echo ""
  echo "## Findings:"
  ...
  echo "4. ROE=4 FAILS to retreat at 0.33 ratio ❌"
  echo ""
  echo "## Potential Bug:"
  echo "ROE=4 with 3:1 disadvantage (0.33 ratio) should retreat"
  echo "Threshold for ROE=4 is 2.0 (per retreat.nim:19)"
  echo "0.33 < 2.0, but no retreat occurred"
  echo ""
  echo "Possible causes:"
  echo "- Fleet survives 2 rounds but doesn't check retreat?"
  echo "- Prestige calculation affecting effective ROE?"
  echo "- evaluateRetreat() not being called correctly?"
  echo ""
  echo "## Recommendation:"
  echo "Investigate retreat.nim:42-114 evaluateRetreat() logic"
  echo "Check engine.nim:125-161 retreat evaluation and execution"
  echo "Add debug logging to see actual retreat evaluations"
