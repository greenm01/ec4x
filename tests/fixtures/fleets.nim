...
proc testFleet_BalancedCapital*(owner: HouseId, location: SystemId, techLevel: int = 1): seq[CombatSquadron] =
  ...
  result = @[]
  for i in 1..2:
    let battleship = newShip(ShipClass.Battleship, techLevel = techLevel)
  ...
  let cruiser = newShip(ShipClass.Cruiser, techLevel = techLevel)
...
proc testFleet_Dreadnought*(owner: HouseId, location: SystemId, techLevel: int = 3): seq[CombatSquadron] =
  ## Single dreadnought
  let dreadnought = newShip(ShipClass.Dreadnought, techLevel = techLevel)
