...
##
## Tests complete fighter capacity system per assets.md:2.4.1 and economy.md:3.10
## **UPDATED:** Now uses IU-based capacity (Industrial Units), not PU-based

import std/[unittest, tables, math]
import ../../src/engine/economy/capacity/fighter
import ../../src/engine/[gamestate, state_helpers]
import ../../src/engine/economy/types as econ_types
import ../../src/common/types/core

proc createTestColony(iu: int, fdLevel: int = 1, fighters: int = 0): Colony =
  result = Colony(
    systemId: "system-1",
    owner: "house-test",
    populationUnits: 500,  # No longer matters for capacity
    industrial: econ_types.IndustrialCapacity(units: iu),  # IU is what matters now
    fighterSquadrons: @[],
...
suite "Max Fighter Capacity Calculation (IU-based)":
  test "100 IU with FD I = 1 FS":
    # floor(100/100) × 1.0 = 1
    let cap = calculateMaxFighterCapacity(100, fdLevel=1)
    check cap == 1

  test "200 IU with FD I = 2 FS":
    let cap = calculateMaxFighterCapacity(200, fdLevel=1)
    check cap == 2

  test "99 IU with FD I = 0 FS":
    # floor(99/100) × 1.0 = 0
    let cap = calculateMaxFighterCapacity(99, fdLevel=1)
    check cap == 0

  test "100 IU with FD II = 1 FS":
    # floor(100/100) × 1.5 = 1 × 1.5 = 1
    let cap = calculateMaxFighterCapacity(100, fdLevel=2)
    check cap == 1

  test "200 IU with FD II = 3 FS":
    # floor(200/100) × 1.5 = 2 × 1.5 = 3
    let cap = calculateMaxFighterCapacity(200, fdLevel=2)
    check cap == 3

  test "300 IU with FD II = 4 FS":
    # floor(300/100) × 1.5 = 3 × 1.5 = 4 (floor)
    let cap = calculateMaxFighterCapacity(300, fdLevel=2)
    check cap == 4

  test "100 IU with FD III = 2 FS":
    # floor(100/100) × 2.0 = 2
    let cap = calculateMaxFighterCapacity(100, fdLevel=3)
    check cap == 2

  test "500 IU with FD III = 10 FS":
    # floor(500/100) × 2.0 = 10
    let cap = calculateMaxFighterCapacity(500, fdLevel=3)
    check cap == 10

  test "Homeworld example (420 IU, FD I = 4 FS)":
    # Per assets.md:2.4.1 example
    let cap = calculateMaxFighterCapacity(420, fdLevel=1)
    check cap == 4

  test "Fully industrialized (840 IU, FD I = 8 FS)":
    # Per assets.md:2.4.1 example
    let cap = calculateMaxFighterCapacity(840, fdLevel=1)
    check cap == 8

  test "840 IU with FD II = 12 FS":
    # Per assets.md:2.4.1 example
    let cap = calculateMaxFighterCapacity(840, fdLevel=2)
    check cap == 12

  test "840 IU with FD III = 16 FS":
    # Per assets.md:2.4.1 example
    let cap = calculateMaxFighterCapacity(840, fdLevel=3)
    check cap == 16

suite "Capacity Analysis (Industrial-based)":
  test "No violation - within IU limits":
    var state = GameState()
    ...
    # 200 IU = 2 FS capacity, have 1 FS
    let colony = createTestColony(iu=200, fdLevel=1, fighters=1)
    ...
    check status.industrialUnits == 200
    check status.excessFighters == 0

  test "Industrial violation - too many fighters for IU":
    var state = GameState()
    state.houses = initTable[HouseId, House]()
    state.houses["house-test"] = House(
      id: "house-test",
    ...
    # 100 IU = 1 FS max, have 3 FS
    let colony = createTestColony(iu=100, fdLevel=1, fighters=3)
    let status = analyzeCapacity(state, colony, "house-test")

    check status.violationType == ViolationType.Industrial
    check status.maxCapacity == 1
    check status.currentFighters == 3
    check status.excessFighters == 2

  test "FD II increases capacity":
    var state = GameState()
    state.houses = initTable[HouseId, House]()
    state.houses["house-test"] = House(
      id: "house-test",
      techTree: TechTree(levels: TechLevels(fighterDoctrine: 2))
    )

    # 200 IU with FD II = 3 FS capacity, have 2 FS = no violation
    let colony = createTestColony(iu=200, fdLevel=2, fighters=2)
    let status = analyzeCapacity(state, colony, "house-test")

    check status.violationType == ViolationType.None
    check status.maxCapacity == 3

suite "Check Violations Batch":
  test "Find all IU violations across colonies":
    var state = GameState()
    state.houses = initTable[HouseId, House]()
    ...
    var colony1 = createTestColony(iu=200, fdLevel=1, fighters=1)
    colony1.systemId = "system-1"
    colony1.owner = "house-test"
    state.colonies["system-1"] = colony1

    # Colony 2: Industrial violation
    var colony2 = createTestColony(iu=100, fdLevel=1, fighters=3)
    colony2.systemId = "system-2"
    colony2.owner = "house-test"
    state.colonies["system-2"] = colony2

    # Colony 3: Another violation
    var colony3 = createTestColony(iu=200, fdLevel=1, fighters=5)
...
suite "Violation Tracking":
  test "New violation starts grace period":
    ...
    var colony = createTestColony(iu=100, fdLevel=1, fighters=6)
    ...
    check state.colonies["system-1"].capacityViolation.violationType == "industrial"
  ...
  test "Resolved violation clears tracking":
    ...
    var colony = createTestColony(iu=200, fdLevel=1, fighters=1)
    colony.systemId = "system-1"
    colony.owner = "house-test"
    colony.capacityViolation.active = true
...
suite "Enforcement Planning":
  test "Plan enforcement after grace period":
    ...
    check action.fightersToDisband.len == 5  # 5 excess fighters (have 6, cap is 1)
  ...
  test "Disband oldest fighters first":
    ...
    var colony = Colony(
      ...
      owner: "house-test",
      populationUnits: 500,
      industrial: econ_types.IndustrialCapacity(units: 100),  # 1 FS capacity
      fighterSquadrons: @[
...
suite "Enforcement Application":
  test "Apply enforcement disbands fighters":
    ...
    var colony = createTestColony(iu=100, fdLevel=1, fighters=3)
    colony.systemId = "system-1"
    colony.owner = "house-test"
    colony.capacityViolation.active = true
    state.colonies["system-1"] = colony
    ...
    let action = EnforcementAction(
      ...
      violationType: ViolationType.Industrial,
...
suite "Can Commission Fighter Check":
  ...
  test "Cannot commission when at capacity":
    ...
    let colony = createTestColony(iu=100, fdLevel=1, fighters=1)
...
when isMainModule:
  echo "Running fighter capacity enforcement tests (IU-based)..."
