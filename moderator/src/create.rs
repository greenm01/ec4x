...
use moderator::config::load_config;

pub fn new_game(path: &PathBuf) -> Result<()> {
    let game_config = load_config(path.clone())?;

    println!("\n##################################");
    println!("##### Creating New EC4X Game #####");
    println!("##################################");
    ...
    //let starmap: StarMap = gen_starmap(game_config.num_empires);
    //gen_starmap(game_config.num_empires);
    gen_starmap();

    Ok(())
}

pub fn gen_starmap() -> Result<()> {
    let player_count = 3;
